...
const NewQuotation = ({
  ...
  const togglePeopleTable = (evt) => {
    ...
    if (pt.classList.contains("scale-in")) {
      pt.classList.remove("scale-in");
      // pt.classList.add("scale-out");

      qg.classList.remove("m8", "l9");
  ...
  const toggleSmallPeopleTable = (evt) => {
    evt.preventDefault();
    let pt = document.getElementById("smallPeopleTable");
    if (pt.classList.contains("hide")) {
  ...
  const checkCreateDisabled = () => {
    ...
    if (!selectedQuotation.modules || selectedQuotation.modules.length === 0)
      return true;
    if (
      selectedQuotation.quotationType === QUOTATION_TYPES.PROVIDER &&
      !selectedQuotation.provider
    )
      return true;
          ...
          <div>
            <form className="white" onSubmit={(e) => saveQuotation(e)}>
              ...
              <div className="row">
                ...
                <div className="col s1 offset-s11 hide-on-med-and-up">
                  <a href="#!" id="smallPeopleTableTrigger" onClick={(evt) => toggleSmallPeopleTable(evt)}>
                    <i
                      className="material-icons indigo-text right"
                      title="People Fees Table"
                    >
              ...
              <div className="row">
                <div
                  className="col s12 hide hide-on-med-and-up"
                  id="smallPeopleTable"
                >
                ...
                <div className="col s12" id="quotationGroup">
                  ...
                  <div className="col s12 m4 l4 z-depth-1 qtCost">
                    <h6 className="bolder price center">
                      Price without PT{" "}
                      {selectedQuotation.quotationNotPTCost || 0}
                    </h6>
                  </div>
                  <div className="col s12 m4 l4 z-depth-1 qtCost">
                    <h6 className="bolder price center">
                ...
                <div
                  className="col m4 l3 scale-transition scale-out hide-on-small-only"
                  id="peopleTable"
                >
                  <ResourceList resources={resources} />
                </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    saveQuotationToDb: (selectedQuotation) =>
      dispatch(addQuotation(selectedQuotation)),
    startNewQuotation: (type) => dispatch(startNewQuotation(type)),
    editQuotationCode: (quotationCode) =>
      dispatch(editSelectedQuotationCode(quotationCode)),
